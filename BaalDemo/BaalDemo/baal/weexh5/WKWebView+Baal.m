...
//
#import "WKWebView+Baal.h"
#import "Baal_ConfigurationDefine.h"
#import "WKWebView+BaalJavaScriptAlert.h"
...
//    NSLog(@"%s",__FUNCTION__);
    ...
    [self removeObserver:self forKeyPath:kBaal_WK_title];
    [self removeObserver:self forKeyPath:kBaal_WK_estimatedProgress];
    [self removeObserver:self forKeyPath:kBaal_WK_URL];
    if ( self.ba_web_isAutoHeight )
    {
        [self.scrollView removeObserver:self forKeyPath:kBaal_WK_contentSize];
    }
}
...
- (void)ba_web_addNoti
{
    ...
    [self addObserver:self
                   forKeyPath:kBaal_WK_title
                      options:NSKeyValueObservingOptionNew
                      context:nil];
    
    // 当前页面载入进度
    [self addObserver:self
                   forKeyPath:kBaal_WK_estimatedProgress
                      options:NSKeyValueObservingOptionNew
                      context:nil];
    
    // 监听 URL，当之前的 URL 不为空，而新的 URL 为空时则表示进程被终止
    [self addObserver:self
           forKeyPath:kBaal_WK_URL
              options:NSKeyValueObservingOptionNew
              context:nil];
}
...
{
    if ([keyPath isEqualToString:kBaal_WK_title])
    {
    ...
    else if ([keyPath isEqualToString:kBaal_WK_estimatedProgress])
    {
...
//        NSLog(@"progress: %f", self.estimatedProgress);
    ...
    else if ([keyPath isEqualToString:kBaal_WK_URL])
    {
...
//            [self reload];
    ...
    else if ( [keyPath isEqualToString:kBaal_WK_contentSize] && [object isEqual:self.scrollView] )
    {
...
//    NSLog(@"URL query: %@", url_query);
    ...
    {
        if ([Baal_SharedApplication canOpenURL:url])
        {
            [Baal_SharedApplication openURL:url];
            decisionHandler(WKNavigationActionPolicyCancel);
            return;
        }
    }
...
//    NSLog(@"isMainframe :%d", isMainframe);
    ...
    else if ([Baal_SharedApplication canOpenURL:url])
    {
        decisionHandler(WKNavigationActionPolicyCancel);
        return;
    }
...
+ (void)load
{
//    Baal_Objc_exchangeMethodAToB(NSSelectorFromString(@"init"), @selector(ba_web_init));
//    Baal_Objc_exchangeMethodAToB(NSSelectorFromString(@"initWithFrame"), @selector(ba_web_initWithFrame));
    Baal_Objc_exchangeMethodAToB(NSSelectorFromString(@"dealloc"),
...
{
    Baal_Objc_setObj(@selector(webViewHeigt), @(webViewHeigt));
}

- (CGFloat)webViewHeigt
{
    return [Baal_Objc_getObj floatValue];
}

- (void)setBa_web_isAutoHeight:(BOOL)ba_web_isAutoHeight
{
    Baal_Objc_setObj(@selector(ba_web_isAutoHeight), @(ba_web_isAutoHeight));
    
    if ( ba_web_isAutoHeight )
    {
        // 监听高度变化
        [self.scrollView addObserver:self
                          forKeyPath:kBaal_WK_contentSize
                             options:NSKeyValueObservingOptionNew
                             context:nil];
    }
}
...
- (BOOL)ba_web_isAutoHeight {
    return [Baal_Objc_getObj boolValue];
}
...
- (void)setBa_web_didStartBlock:(Baal_webView_didStartProvisionalNavigationBlock)ba_web_didStartBlock
{
    Baal_Objc_setObjCOPY(@selector(ba_web_didStartBlock), ba_web_didStartBlock);
}

- (Baal_webView_didStartProvisionalNavigationBlock)ba_web_didStartBlock
{
    return Baal_Objc_getObj;
}

- (void)setBa_web_didCommitBlock:(Baal_webView_didCommitNavigationBlock)ba_web_didCommitBlock
{
    Baal_Objc_setObjCOPY(@selector(ba_web_didCommitBlock), ba_web_didCommitBlock);
}

- (Baal_webView_didCommitNavigationBlock)ba_web_didCommitBlock
{
    return Baal_Objc_getObj;
}

- (void)setBa_web_didFinishBlock:(Baal_webView_didFinishNavigationBlock)ba_web_didFinishBlock
{
    Baal_Objc_setObjCOPY(@selector(ba_web_didFinishBlock), ba_web_didFinishBlock);
}

- (Baal_webView_didFinishNavigationBlock)ba_web_didFinishBlock
{
    return Baal_Objc_getObj;
}

- (void)setBa_web_didFailBlock:(Baal_webView_didFailProvisionalNavigationBlock)ba_web_didFailBlock
{
    Baal_Objc_setObjCOPY(@selector(ba_web_didFailBlock), ba_web_didFailBlock);
}

- (Baal_webView_didFailProvisionalNavigationBlock)ba_web_didFailBlock
{
    return Baal_Objc_getObj;
}

- (void)setBa_web_isLoadingBlock:(Baal_webView_isLoadingBlock)ba_web_isLoadingBlock
{
    Baal_Objc_setObjCOPY(@selector(ba_web_isLoadingBlock), ba_web_isLoadingBlock);
}

- (Baal_webView_isLoadingBlock)ba_web_isLoadingBlock
{
    return Baal_Objc_getObj;
}

- (void)setBa_web_getTitleBlock:(Baal_webView_getTitleBlock)ba_web_getTitleBlock
{
    Baal_Objc_setObjCOPY(@selector(ba_web_getTitleBlock), ba_web_getTitleBlock);
}

- (Baal_webView_getTitleBlock)ba_web_getTitleBlock
{
    return Baal_Objc_getObj;
}

- (void)setBa_web_userContentControllerDidReceiveScriptMessageBlock:(Baal_webView_userContentControllerDidReceiveScriptMessageBlock)ba_web_userContentControllerDidReceiveScriptMessageBlock
{
    Baal_Objc_setObjCOPY(@selector(ba_web_userContentControllerDidReceiveScriptMessageBlock), ba_web_userContentControllerDidReceiveScriptMessageBlock);
}

- (Baal_webView_userContentControllerDidReceiveScriptMessageBlock)ba_web_userContentControllerDidReceiveScriptMessageBlock
{
    return Baal_Objc_getObj;
}

- (void)setBa_web_decidePolicyForNavigationActionBlock:(Baal_webView_decidePolicyForNavigationActionBlock)ba_web_decidePolicyForNavigationActionBlock
{
    Baal_Objc_setObjCOPY(@selector(ba_web_decidePolicyForNavigationActionBlock), ba_web_decidePolicyForNavigationActionBlock);
}

- (Baal_webView_decidePolicyForNavigationActionBlock)ba_web_decidePolicyForNavigationActionBlock
{
    return Baal_Objc_getObj;
}

- (void)setBa_web_getCurrentUrlBlock:(Baal_webView_getCurrentUrlBlock)ba_web_getCurrentUrlBlock
{
    Baal_Objc_setObjCOPY(@selector(ba_web_getCurrentUrlBlock), ba_web_getCurrentUrlBlock);
}

- (Baal_webView_getCurrentUrlBlock)ba_web_getCurrentUrlBlock
{
    return Baal_Objc_getObj;
}

- (void)setBa_web_getCurrentHeightBlock:(Baal_webView_getCurrentHeightBlock)ba_web_getCurrentHeightBlock
{
    Baal_Objc_setObj(@selector(ba_web_getCurrentHeightBlock), ba_web_getCurrentHeightBlock);
}

- (Baal_webView_getCurrentHeightBlock)ba_web_getCurrentHeightBlock
{
    return Baal_Objc_getObj;
}
...
{
    Baal_Objc_setObj(@selector(ba_web_urlScheme), ba_web_urlScheme);
}
